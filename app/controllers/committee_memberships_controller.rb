class CommitteeMembershipsController < ApplicationController
  ...
  def create
    ...
    if @committee.committee_memberships.create!(user: user)
      flash[:success] = "User #{user.name} added to committee #{@committee.name}."
      redirect_to committee_path(@committee)
    else
      flash[:error] = "User #{user.name} not added to committee #{@committee.name}."
      redirect_to committee_path(@committee)
    end
  end
  ...
  def destroy
    ...
    flash[:success] = "User #{user.name} removed from committee #{@committee.name}."
    redirect_to committee_path(@committee)
  end
